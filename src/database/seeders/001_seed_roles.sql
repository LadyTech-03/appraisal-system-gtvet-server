-- Insert default roles and permissions
-- This is handled in the application code, but keeping for reference

-- Note: Roles are managed in the application code as constants
-- The following roles are available:

-- - DIVISIONS

-- - MANAGEMENT DIVISION

-- - Director-General
-- - Deputy Director-General, Management Services
-- - Deputy Director-General, Operations
-- - Internal Audit
-- - Legal Services
-- - Corporate Affairs

-- - OTHER DIVISIONS

-- - HR Management and Development Division Head
-- - Finance Division Head
-- - Administration Division Head
-- - Research, Innovation, Monitoring & Evaluation Division Head
-- - EduTech Division Head
-- - Infrastructure Planning & Development Division Head
-- - Apprenticeship Division Head
-- - Partnerships,WEL and Inclusion Division Head
-- - Training Assessment and Quality Head
-- - Regional Director

-- - UNITS

-- - HR Planning & Analysis Unit Head
-- - Talent Management & Development Unit Head
-- - Accounting & Financial Reporting Unit Head
-- - Budgeting, Disbursement & Treasury Unit Head
-- - General Admin Unit Head
-- - Transport Unit Head
-- - Records Management Unit Head
-- - Library Unit Head
-- - Procurement Unit Head
-- - Policy Planning Unit Head
-- - Research & Innovation Unit Head
-- - Monitoring & Evaluation Unit Head
-- - System Access & Educational Delivery Unit Head
-- - Management Information Systems Unit Head
-- - Planning & Development Unit Head
-- - Facilities Management Unit Head
-- - Apprenticeship Skills Delivery Unit Head
-- - Apprenticeship Coordination Unit Head
-- - Partnership & Resource Mobilisation Unit Head
-- - WEL Unit Head
-- - Diversity & Inclusive Education Unit Head
-- - Training & Development Unit Head
-- - Assessment & Quality Assurance Unit Head
-- - Carrer Guidance & Counselling Unit Head
-- - Technical Institutions Unit Head

-- - Supervisor
-- - Staff Officer
